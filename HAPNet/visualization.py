...
def visualization(net, data, save_path, device, color_map, size):
    ...
    for hsi, sar, i, j in tqdm(data):
        hsi = hsi.to(device)
        sar = sar.to(device)
        output, hsi_feature, hsi_compensation, sar_feature, sar_compensation = net(hsi, sar)
...
def visHouston2013(net, data, save_path, device):
    ...
    visualization(net, data, save_path, device,
                  houston2013_color_map, houston2013_size)
...
def visHouston2018(net, data, save_path, device):
    ...
    visualization(net, data, save_path, device,
                  houston2018_color_map, houston2018_size)
...
def visAugsburg(net, data, save_path, device):
    ...
    visualization(net, data, save_path, device,
                  augsburg_color_map, augsburg_size)
