...
class WindowAttention(Attention):
    ...
    def forward(self, qkv, x_size, table, index, mask):
        ...
        B_, N, _ = qkv.shape
        #print(qkv.shape, 3, self.num_heads)
...
class MyAttention(nn.Module):
    ...
    def forward(self, x, x_size, all_table_index_mask):
        ...
        """
        B, L, C = x.shape
        table_index_mask = self._get_all_table_index_mask(all_table_index_mask)
        # qkv projection
        qkv = self.qkv(x, x_size)
        #print('qkv0', qkv.size())
        qkv_window, qkv_stripe = torch.split(qkv, C * 3 // 2, dim=-1)
        # anchor projection
        #print('qkv00', qkv_window.size())
    ...
    def _get_all_table_index_mask(self, all_table_index_mask):
        table_index_mask = {
...
class newAttention(nn.Module):
    ...
    def forward(self, x, x_size, all_table_index_mask):
        ...
        if self.type == 'G':
            ...
            x = self.window_attn(
                qkv, x_size, *self._get_table_index_mask(table_index_mask, True)
            )
        # output projection
        x = self.proj(x)
        x = self.proj_drop(x)
