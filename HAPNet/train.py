from models.networks.grnet import MyNet
#from models.networks.GAnet import *
import torch.optim as optim
from dataset import *
#from models.networks.gpwa import *
import time
...
def train(epochs, lr, model, cuda, train_loader, test_loader, out_features, model_savepath, log_path):
    ...
    if model == 'MyNet':
        net = MyNet(out_features).to(device)
    ...
    for epoch in range(epochs):
        ...
        #     i = i + 1
        #     print(name, ':', parameters[0][0])
        #     if(i>1):
        #         break
        getLog(log_path, log)
    # src = '../model/net.pth'
    # dst = '../model/net_{}.pth'.format(max_acc)
    # os.rename(src, dst)
...
def myTrain(datasetType, model):
    ...
    train(epoch_nums, lr, model, cuda, train_loader, test_loader, out_features[datasetType],
          model_savepath[datasetType], log_path[datasetType])
